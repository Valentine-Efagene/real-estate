...
- Husky errors seem to be wasting tokens. Remove all husky settings and scripts to avoid this issue. The token cost of husky errors is not worth the benefit of pre-commit hooks in this case. Another thing to do is to use the `--ignore-scripts` flag, to prevent third party dependencies from bringing up the error as well.
