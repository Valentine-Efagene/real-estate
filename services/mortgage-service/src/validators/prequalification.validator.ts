...
export const CreatePrequalificationSchema = z
    .object({
        propertyId: z.string().min(1).openapi({ example: 'prop-id-123' }),
        paymentMethodId: z.string().min(1).openapi({ example: 'pm-id-123' }),
        answers: z.union([
            z.record(z.string(), z.any()),
            z.array(z.any())
        ]).optional().openapi({ example: { employmentType: 'EMPLOYED', yearsEmployed: 5 } }),
...
export const SubmitDocumentSchema = z
    .object({
        ...
        url: z.string().url().openapi({ example: 'https://s3.amazonaws.com/bucket/doc.pdf' }),
        fileName: z.string().optional().openapi({ example: 'passport.pdf' }),
        mimeType: z.string().optional().openapi({ example: 'application/pdf' }),
        sizeBytes: z.number().int().positive().optional().openapi({ example: 1024000 }),
    })
...
export const ReviewPrequalificationSchema = z
    .object({
        status: z.enum(['APPROVED', 'REJECTED']).optional().openapi({ example: 'APPROVED' }),
        decision: z.enum(['APPROVED', 'REJECTED']).optional().openapi({ example: 'APPROVED' }),
    ...
    .transform((data) => ({
        ...data,
        status: data.status || data.decision!, // Normalize decision to status
    }))
